...
# Invokes the plugin's own `build:static` script (the same one plugin-ci runs
# via prepack), so the build recipe lives in one place — plugins/studio/package.json.
...
# Plugin sources for `file:` deps in package.json — needed at install time
# because @trex/pg-meta and @trex/studio aren't published to the registry yet.
# Removed after install; real plugin loading goes via node_modules/@trex (the
# ./plugins/@trex symlink below) and plugins-dev/ (lines further down).
# build_static/ comes from the studio-builder stage so npm install's prepack
# is a no-op (idempotent: it skips when build_static/index.html exists).
...
# Studio plugin: Next.js static export is generated by the studio-builder
# stage (TREX_STATIC_EXPORT=1 next build, via the plugin's own build:static
# script) and injected as build_static/. The plugin loader (core/server) serves
# it at /usr/src/plugins-dev/studio/build_static/.
