...
| `auth.ts` | `auth.js` | **Reimplemented (flagged).** eve mints the App JWT with `node:crypto` `createSign("RSA-SHA256")`, so the RS256 signing is redone on **WebCrypto** RSASSA-PKCS1-v1_5 + SHA-256 (importing the key via `importKey("pkcs8", …)`). This is the ONE place the crypto backend differs from eve — exercised by github.test.ts (JWT header/claims shape + a full mock mint→exchange→public-key-verify round-trip). **KEY FORMAT (deployment fix):** WebCrypto's `importKey("pkcs8", …)` accepts ONLY PKCS8 (`-----BEGIN PRIVATE KEY-----`), but GitHub Apps download the key in PKCS1 (`-----BEGIN RSA PRIVATE KEY-----`). Node's `createSign` accepted both transparently; the reimplementation would silently break the GitHub key with an opaque "operation does not meet requirements" error, so a PKCS1 PEM is DETECTED and CONVERTED to PKCS8 in-process (the deterministic `openssl pkcs8 -topk8`-equivalent ASN.1 wrapping: prepend the `rsaEncryption` AlgorithmIdentifier + version, re-wrap the PKCS1 DER in the PKCS8 SEQUENCE/OCTET STRING) before import; PKCS8 keys pass through as-is. Both formats are tested (the PKCS1-converted signature verifies against the original public key). Everything else — the header/claims (`{alg:"RS256",typ:"JWT"}` / `{iss, iat:now-60, exp:now+600}`), the `/app/installations/{id}/access_tokens` exchange, the 60s-skew installation-token cache, and the `GITHUB_APP_ID`/`GITHUB_APP_PRIVATE_KEY`/`GITHUB_APP_INSTALLATION_ID` resolvers — is eve's, unchanged. `process.env` → `getEnv`. |
...
| `inbound.ts` | `inbound.js` | **Reimplemented.** eve's `parseLinearWebhookEvent` is PURE (imports only `#shared/*`) but shaped around the Agent Session model; its non-session branch collapses every other delivery into an opaque `{ kind:"data", type, action, raw }`. This file keeps eve's data-webhook CLASSIFICATION (`type`/`action`/`data`, the `Linear-Event`/`Linear-Delivery` headers, `organizationId`) but ADDS the Comment/Issue field extraction, the `isIgnoredLinearEvent` **config-free loop guard**, the raw issue-id continuation token, and the `<linear_context>` block. eve's agent-session readers (`readAgentSession`/`readAgentActivity`/…) and its `agent-session:` token are NOT carried. **LOOP GUARD (config-free, trex):** eve's agent-session model never re-ingests its own activities, so eve has no comment loop guard; the classic comment model DOES (Linear echoes the bot's `commentCreate` back as a `Comment.create` webhook). Drops an event when ANY of: (1) the body carries the `LINEAR_TREX_MARKER` the adapter stamps on EVERY outgoing comment (works with NO bot-id config); (2) it is APP-AUTHORED — `data.botActor` present with no human author, i.e. posted by our OAuth agent/app token, `data.user` null (also config-free); (3) the human actor id equals a configured `botUserId`. `readLinearActor` reads `data.botActor` as a fallback after the human sources. `parseLinearWebhookEvent` also accepts an already-parsed `payload` so verify's parse is reused (no double JSON.parse). |
| `verify.ts` (parse-once) | — | Beyond the WebCrypto HMAC (above), `verifyLinearInbound` now returns `{ body, payload }` — the payload is JSON-parsed ONCE (for the timestamp check) and threaded to `parseLinearWebhookEvent`, so the route no longer parses the body twice. The verifier path leaves `payload` undefined (its possibly-rewritten body is parsed downstream). |
| `api.ts` | `api.js` | **Partly vendored**, de-minified. **Vendored:** the GraphQL transport `callLinearGraphQL` (POST `https://api.linear.app/graphql`, JSON body, `errors[]`/non-2xx → `LinearApiError`) + the `LinearApiError` class — eve's, unchanged (imports map to siblings, no eve import survives). **Trex-added (NOT vendored):** `createLinearComment` (the `commentCreate($input: CommentCreateInput!)` mutation). eve has no `commentCreate` — it posts Agent Activities (`agentActivityCreate`) — so the mutation is trex's over eve's transport. eve's agent-session mutations are DROPPED (YAGNI). **AUTH-HEADER FIX (diverges from eve):** eve hardcodes `Authorization: Bearer <token>` (it only handles the OAuth agent token). Linear uses TWO schemes — a personal API key (`lin_api_…`, the brief's `LINEAR_API_KEY`) must be sent RAW (`Bearer lin_api_…` 401s), an OAuth agent token uses `Bearer`. `linearAuthorization` picks the scheme by the `lin_api_` prefix so both credential kinds work. |
