// Reimplemented for trex — there is NO usable eve source to vendor. eve's Linear
// `hitl.js` encodes input requests as a base64url `<!-- eve-input: … -->` marker
// packed into an Agent Activity and resolves the reply via the runtime-coupled
// `#channel/resolve-text` + a `listAgentSessionActivities` lookup — i.e. eve
// runtime code tied to the Agent Session model. The trex Linear channel follows
// the classic comment model, so (like `github/hitl.ts` and `twilio/hitl.ts`) it
// renders an input request as a plain-text reply-instructions comment (Linear
// has no interactive widgets) and maps the user's next comment back to an option
// with a stateless text match. See vendor/VENDOR.md.

import type { InputOption, InputRequest, InputResponse } from "./shared.ts";

/**
 * Renders an input request as a Linear comment body: the prompt, a numbered list
 * of options each with a `/slash` reply command, and a one-line instruction.
 * Text-only (no widgets).
 */
export function renderLinearInputRequest(request: InputRequest): string {
  const options = request.options ?? [];
  const lines: string[] = [request.prompt, ""];
  options.forEach((opt, i) => {
    lines.push(`${i + 1}. **${opt.label}** — reply with \`/${opt.id}\` or \`${i + 1}\``);
  });
  if (options.length > 0) {
 ...
 */
export function deriveLinearInputResponse(body: string, request: InputRequest): InputResponse | null {
  const options = request.options ?? [];
