// Vendored from eve@0.19.0 dist/src/public/channels/telegram/inbound.js
// (Apache-2.0). Modified: imports rewritten from `#shared/guards` to the
// sibling `./shared.ts`; types added from the paired inbound.d.ts for
// `deno check`; de-minified. Parsing logic unchanged. See vendor/VENDOR.md.

import { isNonEmptyString, isObject } from "./shared.ts";

/** Telegram chat types the channel recognizes. */
...
/** Discriminated union of inbound updates the channel handles. */
...
/** Inbound identity and response guidance for the model-visible context block. */
...
export function parseTelegramUpdate(value: unknown): TelegramUpdate | null {
  ...
  return callbackQuery === null ? null : { kind: "callback_query", callbackQuery };
}

/** Renders one context block with fixed response instructions + chat/user identity. */
export function formatTelegramContextBlock(context: TelegramInboundContext): string {
  return [
    ...
    ...(context.messageThreadId === undefined ? [] : [`message_thread_id: ${context.messageThreadId}`]),
...
function parseTelegramMessage(value: unknown): TelegramMessage | null {
  ...
  return {
    ...
    messageId,
    messageThreadId: typeof value.message_thread_id === "number" ? value.message_thread_id : undefined,
    raw: value,
...
function parseAttachments(value: Record<string, unknown>): TelegramAttachment[] {
  ...
  if (document !== null) out.push(document);
  return out;
}
...
function parseLargestPhoto(value: unknown): TelegramAttachment | null {
  ...
  const best = value
    .filter(isObject)
    .map((p) => ({
      fileId: typeof p.file_id === "string" ? p.file_id : "",
      fileUniqueId: typeof p.file_unique_id === "string" ? p.file_unique_id : undefined,
      height: typeof p.height === "number" ? p.height : undefined,
      size: typeof p.file_size === "number" ? p.file_size : undefined,
      width: typeof p.width === "number" ? p.width : undefined,
    }))
    .filter((p) => p.fileId.length > 0)
    .sort((a, b) => scorePhoto(b) - scorePhoto(a))[0];
  return best
    ? {
      fileId: best.fileId,
      fileName: "photo.jpg",
...
function scorePhoto(p: { size?: number; width?: number; height?: number }): number {
  return p.size === undefined ? (p.width ?? 0) * (p.height ?? 0) : p.size;
}
...
export function parseTelegramChatType(value: unknown): TelegramChatType | null {
  return value === "channel" || value === "group" || value === "private" || value === "supergroup" ? value : null;
}
