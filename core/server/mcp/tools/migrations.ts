...
export function registerMigrationTools(server: McpServer) {
  server.tool(
    "migration-list",
    "List migration status for the core trex schema. Shows each migration's version number, name, status (applied/pending), applied timestamp, and checksum. Use this to check if there are pending migrations that need to be run.",
    {},
    async () => {
      try {
  ...
  server.tool(
    "migration-run",
    "Run pending core schema migrations against the trex schema in _config.",
    {},
    async () => {
      try {
        const conn = new Trex.TrexDB("memory");
