...
impl ModuleGraphBuilder {
  ...
  fn resolve_jsx_import_source_config(&self) -> Option<JsxImportSourceConfig> {
    let root_config = self.options.workspace().root_deno_json()?;
    let compiler_options = root_config
      .json
      .compiler_options
      .as_ref()?
      .as_object()?;

    let jsx = compiler_options.get("jsx")?.as_str()?;
    let module = match jsx {
      "react-jsx" | "preserve" => "jsx-runtime",
      "react-jsxdev" => "jsx-dev-runtime",
      "precompile" => "jsx-runtime",
      _ => return None, // classic "react" doesn't use import source
    };

    let base = root_config.specifier.clone();
    let import_source_str = compiler_options
      .get("jsxImportSource")
      .and_then(|v| v.as_str())
      .unwrap_or("react");

    let import_source = Some(JsxImportSourceSpecifierConfig {
      specifier: import_source_str.to_string(),
      base: base.clone(),
    });
