...
impl DuckDbClient {
  pub fn trexdb(
    ...
    let _ = conn.lock().unwrap().execute(
      &format!(
        "ATTACH IF NOT EXISTS './data/cache/{file_name}.db' AS {file_name}"
      ),
      [],
    );
  ...
  pub fn insert_row(
    ...
    let table_name = format!(
      "{}.{}.{}",
      &self.current_database, table_name.schema, table_name.name
    );
    ...
    let mut stmt = c.prepare(&query)?;
    let _ = stmt
      .execute(params_from_iter(table_row.values.iter()))
      .inspect_err(|e| {
        warn!("TREX: Failled to insert row into {table_name}: {e}")
      });
  ...
  pub fn insert_rows2(
    ...
    let _ = stmt
      ...
      .inspect_err(|e| warn!("TREX: Failled to insert row: {e}"));
  ...
  pub fn insert_rows(
    ...
    for table_row in table_rows {
      let _ = appender
        .append_row(appender_params_from_iter(table_row.values.iter()))
        .inspect_err(|e| warn!("TREX: Failled to insert row (appender): {e}"));
    }
    Ok(())
  }
  ...
  pub fn get_copied_table_ids(
    &self,
  ) -> Result<HashSet<TableId>, duckdb::Error> {
    let c = self.conn.lock().unwrap();
    let mut stmt = c.prepare(&format!(
      "select table_id from {}.pg_replicate.copied_tables",
      &self.current_database
    ))?;
  ...
  pub fn get_last_lsn(&self) -> Result<PgLsn, duckdb::Error> {
    let c = self.conn.lock().unwrap();
    let mut stmt = c.prepare(&format!(
      "select lsn from {}.pg_replicate.last_lsn",
      &self.current_database
    ))?;
  ...
  pub fn set_last_lsn(&self, lsn: PgLsn) -> Result<(), duckdb::Error> {
    ...
    let c = self.conn.lock().unwrap();
    let mut stmt = c.prepare(&format!(
      "update {}.pg_replicate.last_lsn set lsn = ?",
      &self.current_database
    ))?;
    stmt.execute([lsn])?;
    Ok(())
  ...
  pub fn insert_last_lsn_row(&self) -> Result<(), duckdb::Error> {
    self.conn.lock().unwrap().execute(
      &format!(
        "insert into {}.pg_replicate.last_lsn values (0)",
        &self.current_database
      ),
      [],
    )?;
  ...
  pub fn insert_into_copied_tables(
    &self,
    table_id: TableId,
  ) -> Result<(), duckdb::Error> {
    let c = self.conn.lock().unwrap();
    let mut stmt = c.prepare(&format!(
      "insert into {}.pg_replicate.copied_tables values (?)",
      &self.current_database
    ))?;
      ...
      ArrayCell::TimeStamp(mut vec) => {
        let v = vec
          ...
          .map(|v| match v {
            None => Value::Null,
            Some(t) => {
              Value::Text(t.format("%Y-%m-%d %H:%M:%S%.f").to_string())
            }
          })
          .collect();
        Value::Array(v)
      ...
      ArrayCell::TimeStampTz(mut vec) => {
        let v = vec
          ...
          .map(|v| match v {
            None => Value::Null,
            Some(t) => {
              Value::Text(t.format("%Y-%m-%d %H:%M:%S%.f%:z").to_string())
            }
          })
          .collect();
        Value::Array(v)
