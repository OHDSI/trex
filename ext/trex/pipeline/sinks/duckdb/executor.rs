...
use crate::{
  ...
  conversions::table::{ColumnSchema, TableId, TableName, TableSchema},
...
impl DuckDbExecutor {
      ...
      while let Some(req) = self.req_receiver.recv().await {
        match req {
          ...
          DuckDbRequest::HandleCdcEvent(event) => {
            ...
            let committed_lsn =
              self.committed_lsn.expect("committed lsn is none");
  ...
  fn get_resumption_state(
    &self,
  ) -> Result<PipelineResumptionState, DuckDbExecutorError> {
    ...
    self.client.create_table_if_missing(
      &copied_tables_table_name,
  ...
  fn get_table_schema(
    &self,
    table_id: TableId,
  ) -> Result<&TableSchema, DuckDbExecutorError> {
    self
      .table_schemas
      .as_ref()
