...
(deftest postgres-dsn-test
  (testing "standard jdbc:postgresql url → libpq keyword string (values single-quoted)"
  ...
  (testing "query string after dbname is dropped"
    (is (= "host='h' port='5432' dbname='cdm' user='u' password='p'"
           (dsn/postgres-dsn {:jdbc-url "jdbc:postgresql://h:5432/cdm?sslmode=require"
                              :user "u" :password "p"}))))
  (testing "single quotes in a credential are doubled inside the quoted value"
    (is (= "host='h' port='5432' dbname='cdm' user='u' password='p''q'"
           (dsn/postgres-dsn {:jdbc-url "jdbc:postgresql://h:5432/cdm"
                              :user "u" :password "p'q"}))))
  (testing "a value containing a space is safely single-quoted (no DSN-key injection)"
    (is (= "host='h' port='5432' dbname='cdm' user='u' password='p w'"
           (dsn/postgres-dsn {:jdbc-url "jdbc:postgresql://h:5432/cdm"
                              :user "u" :password "p w"})))))
