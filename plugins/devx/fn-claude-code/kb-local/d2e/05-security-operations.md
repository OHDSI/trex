...
Public-bypass list first (health, OAuth callback, OIDC discovery, public listings — operator-extensible regex). Token from `Authorization: Bearer`, falling back to the `authtoken` cookie (needed for WebSocket/Shiny which can't carry custom headers). **JWKS** keys fetched from Logto, cached, periodically refreshed (auto key-rotation, no redeploy). Verifies signature/expiry/audience/issuer; failure → 401.
