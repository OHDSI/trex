...
Each analysis runs asynchronously: `hades_execute` launches a job and returns
immediately with a job id; you poll `hades_status` / `hades_jobs` to track
progress.
...
```sql
SELECT hades_setup_env('/studies/my-study/renv.lock', 'my-study', '/opt/hades-envs');
```

## Functions

### `hades_setup_env(lockfile_path, env_name, base_dir)`
...
### `hades_execute(spec_path, cdm_schema, work_schema, output_path, db_name, env_name, env_base_dir)`
...
| env_name | VARCHAR | R environment to run in (from `hades_setup_env`). |
...
```sql
SELECT * FROM hades_status('job-abc-123');
```

### `hades_jobs()`

List all known jobs (same columns as `hades_status`, without `log_tail`).

**Returns:** TABLE — one row per job.

```sql
SELECT * FROM hades_jobs();
```
...
```sql
SELECT * FROM hades_envs('/opt/hades-envs');
```
...
- **Jobs are asynchronous and process-based.** Each `hades_execute` spawns a
  separate process. Track them with `hades_jobs` / `hades_status` and stop them
  with `hades_cancel`.
