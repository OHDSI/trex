...
Deno.test("assembleBarchart returns categories/measures/totalPatientCount + fills combos", () => {
  const axes = [
    { id: "x1", valueExpr: "AGE", kind: "num", binSize: 10, axisNum: 1 },
    { id: "y1", valueExpr: "GENDER", kind: "text", axisNum: 2 },
  ];
  const rows = [
  ...
  assertEquals(res.categories[1].axis, 2);
  assertEquals(res.measures[0].id, "patient.attributes.pcount");
  assertEquals(res.data.length, 4);
  const filled = res.data.find((d) => d.x1 === 40 && d.y1 === "female");
  assertEquals(filled["patient.attributes.pcount"], 0);
});
