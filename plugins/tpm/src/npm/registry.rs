...
mod tests {
  ...
  fn validate_package_name_rejects_traversal_and_nuls() {
    // Path traversal and shell-metacharacters must be rejected — this name
    // is interpolated into a filesystem path by install_package/delete_package.
    ...
    // Scoped form with extra slash:
    assert!(validate_package_name("@scope/foo/bar").is_err());
    // Bare slash without scope:
    assert!(validate_package_name("foo/bar").is_err());
  }
  ...
  fn assert_path_contained_rejects_escape() {
    ...
    // A sibling of `root` (outside of it) must be rejected.
  ...
  fn list_installed_packages_reads_plain_and_scoped() {
    ...
    // a directory without package.json should be silently ignored
    ...
    let pairs: Vec<(String, String)> = results
      ...
      .collect();
    assert!(
      pairs.contains(&("lodash".to_string(), "4.17.21".to_string())),
      "missing plain pkg in {:?}",
      pairs
    );
    assert!(
      pairs.contains(&("@scope/util".to_string(), "1.2.3".to_string())),
      "missing scoped pkg in {:?}",
      pairs
    );
    assert_eq!(pairs.len(), 2, "orphan dir leaked: {:?}", pairs);
  }
  ...
  fn delete_package_rejects_invalid_name() {
    ...
    assert!(res.is_err(), "expected validation failure, got {:?}", res);
  }
  ...
  fn delete_package_removes_existing_plain_package() {
    ...
    assert!(res.deleted, "expected deleted=true, got {:?}", res);
    assert!(!pkg.exists());
  }
