...
use std::borrow::Cow;
// use std::collections::HashMap; // Removed - only used by env ops
// use std::collections::HashSet; // Removed - only used by env ops
use std::env; // Still needed for env::current_exe() in op_exec_path
use std::sync::Arc;
...
// use once_cell::sync::Lazy; // Removed - only used by env ops
use serde::Serialize;
...
// NODE_ENV_VAR_ALLOWLIST removed - only used by env ops which are handled by ext_env
...
deno_core::extension!(
  deno_os,
  ops = [
    // Most env ops removed - conflicts with ext_env:
    // op_env, op_delete_env, op_get_env, op_set_env
    // BUT op_get_env_no_permission_check is kept for Node.js compatibility
    op_exec_path,
    op_exit,
    ...
    // op_runtime_memory_usage, // Removed - conflicts with ext_runtime
...
// ============================================================================
// Most ENV OPS REMOVED - These conflict with ext_env which provides filtered env access
// ============================================================================
// fn dt_change_notif(isolate: &mut v8::Isolate, key: &str) { ... }
// fn op_set_env(...) -> Result<(), OsError> { ... }
// fn op_env(...) -> Result<HashMap<String, String>, PermissionCheckError> { ... }
// fn op_get_env(...) -> Result<Option<String>, OsError> { ... }
// fn op_delete_env(...) -> Result<(), OsError> { ... }
// ============================================================================
// HOWEVER, op_get_env_no_permission_check and get_env_var are kept for Node.js compatibility:
